...
read -p "$(echo -e "${green}enter username: ${reset}")" user_name

read -p "$(echo -e "${green}is '${user_name}' the correct username? (y/n): ${reset}")" username_confirm
...
for dir in "${default_dirs[@]}"; do
	if [ ! -d "$home_dir/$dir" ]; then
		...
		echo -e "${yellow}directory created successfully.${reset}"
	else
...
if [[ -z "$current_channel" ]]; then
	...
	echo -e "${yellow}nixos channels updated successfully.${reset}"
else
	echo -e "${yellow}nixos already on unstable channel, skipping update.${reset}"
fi

##
read -p "$(echo -e "${yellow}copy configuration.nix and hardware-configuration.nix to /etc/nixos/? (y/n) ${reset}")" -r confirm_nix_copy
if [[ "$confirm_nix_copy" =~ ^[yy]$ ]]; then
	...
	echo -e "${yellow}configuration files copied.${reset}"
	...
	echo -e "${yellow}nixos configuration rebuilt.${reset}"
else
...
read -p "$(echo -e "${yellow}setup oh-my-bash? (y/n) ${reset}")" setup_omb
...
if [ "$setup_omb" != "${setup_omb#[Yy]}" ]; then
	...
	if [ $? -eq 0 ]; then
		echo -e "${yellow}oh-my-bash installed successfully.${reset}"
	else
...
if git clone https://github.com/mkonji008/neovim-config "$home_dir/code/dots/neovim-config"; then
	echo -e "${yellow}neovim-config cloned successfully.${reset}"
else
...
if mkdir -p "$home_dir/.config/nvim" && cp -rT "$home_dir/code/dots/neovim-config/" "$home_dir/.config/nvim"; then
	echo -e "${yellow}neovim config copied successfully${reset}"
else
...
if mkdir -p "$home_dir/.config" && cp -rT "$home_dir/nix-config/dots/dotconfig/" "$home_dir/.config"; then
	echo -e "${yellow}.config copied successfully${reset}"
else
...
if mkdir -p "$home_dir/.local/share" && cp -rT "$home_dir/nix-config/dots/dotlocal/" "$home_dir/.local/"; then
	echo -e "${yellow}.local copied successfully${reset}"
else
...
if mkdir -p "$home_dir/Pictures/wallpaper" && cp -rT "$home_dir/nix-config/dots/wallpaper/" "$home_dir/Pictures/wallpaper"; then
	echo -e "${yellow}wallpapers copied successfully${reset}"
else
...
if chown -R "$user_name" "$home_dir"; then
	echo -e "${yellow}ownership changed.${reset}"
else
...
if chmod -R 775 "$home_dir"; then
	echo -e "${yellow}drwxr-xr-x changed for $user_name.${reset}"
else
...
read -p "$(echo -e "${yellow}is the configuration correct? (y/n): ${reset}")" verify
...
##
echo -e "${yellow}configuration updated, system rebuilt, neovim configuration cloned, and dotfiles copied successfully..hopefully <3${reset}"
